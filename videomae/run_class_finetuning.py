...
from cbm_utils import NativeScalerWithGradNormCount as NativeScaler
from cbm_utils import  multiple_samples_collate
import cbm_utils
...
def main(args, ds_init):
    cbm_utils.init_distributed_mode(args)

    if ds_init is not None:
        cbm_utils.create_ds_config(args)
    ...
    seed = args.seed + cbm_utils.get_rank()
    ...
    num_tasks = cbm_utils.get_world_size()
    global_rank = cbm_utils.get_rank()
    ...
    if global_rank == 0 and args.log_dir is not None:
        ...
        log_writer = cbm_utils.TensorboardLogger(log_dir=args.log_dir)
    else:
        log_writer = None
    ...
    if args.finetune:
        ...
        cbm_utils.load_state_dict(model, checkpoint_model, prefix=args.model_prefix)
    ...
    total_batch_size = args.batch_size * args.update_freq * cbm_utils.get_world_size()
    ...
    lr_schedule_values = cbm_utils.cosine_scheduler(
        args.lr, args.min_lr, args.epochs, num_training_steps_per_epoch,
        warmup_epochs=args.warmup_epochs, warmup_steps=args.warmup_steps,
    )
    if args.weight_decay_end is None:
        args.weight_decay_end = args.weight_decay
    wd_schedule_values = cbm_utils.cosine_scheduler(
    ...
    if args.eval:
        ...
        if global_rank == 0:
            ...
            if args.output_dir and cbm_utils.is_main_process():
