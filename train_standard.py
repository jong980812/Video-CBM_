...
def train_and_save(args):
    ...
    target_save_name, _, _ = cbm_utils.get_save_names("", args.backbone, args.feature_layer, d_train, "", "avg",
                                                  args.activation_dir)
    val_target_save_name, _, _ =  cbm_utils.get_save_names("", args.backbone, args.feature_layer, d_val, "", "avg",
                                                       args.activation_dir)
    #save activations and get save_paths
    if args.backbone.startswith("clip_"):
        model, _ = clip.load(args.backbone[5:], device=args.device)
        cbm_utils.save_clip_image_features(model, data_t, target_save_name, args.batch_size, args.device)
        cbm_utils.save_clip_image_features(model, val_data_t, val_target_save_name, args.batch_size, args.device)
    else:
        cbm_utils.save_target_activations(target_model, data_t, target_save_name, target_layers = [args.feature_layer],
                                  batch_size = args.batch_size, device = args.device, pool_mode='avg')
        cbm_utils.save_target_activations(target_model, val_data_t, val_target_save_name, target_layers = [args.feature_layer],
                                  batch_size = args.batch_size, device = args.device, pool_mode='avg')
