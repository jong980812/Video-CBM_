...
from learning_concept_layer import train_cocept_layer, train_one_stream
...
parser.add_argument('--data_set', default='Kinetics-400', choices=['kinetics100','kinetics400','kinetics400_scratch', 'mini-SSV2','SSV2', 'UCF101', 'HMDB51','TOY','kth','image_folder'],
...
parser.add_argument('--train_mode',type=str,default='single')
...
parser.add_argument('--hard_label', default=None, help='hard label path(.pkl): if you want to use not dual encoder, but hand crafting label')
...
parser.add_argument('--use_mlp',action='store_true', help='if you want to use hard label with MLP')
...
def train_cbm_and_save(args):
    ...
    if args.concept_set==None:
        args.concept_set = "/data/psh68380/repos/Video-CBM_two-stream/data/concept_sets/kth_concepts.txt"
    args.video_anno_path=os.path.join(os.getcwd(),'data/video_annotation',args.data_set)
    # similarity_fn = similarity.cos_similarity_cubed_single # learning_concept_layer.py에 포함되어있음
    ...
    target_save_name, clip_save_name, text_save_name = cbm_utils.get_save_names(args.dual_encoder, args.backbone, 
                                            args.feature_layer,d_train, args.concept_set, "avg", args.activation_dir,args)
    val_target_save_name, val_clip_save_name, text_save_name =  cbm_utils.get_save_names(args.dual_encoder, args.backbone,
                                            args.feature_layer, d_val, args.concept_set, "avg", args.activation_dir,args)
    ...
    with torch.no_grad():
        ...
        #! Internvid image feature
        if args.hard_label is None:
            video_features = torch.load(clip_save_name, map_location="cpu").float()#B,T,D
            video_features /= torch.norm(video_features, dim=1, keepdim=True)
            val_video_features = torch.load(val_clip_save_name, map_location="cpu").float()
            val_video_features /= torch.norm(val_video_features, dim=1, keepdim=True)
            # text feature
            text_features = torch.load(text_save_name, map_location="cpu").float()
            text_features /= torch.norm(text_features, dim=1, keepdim=True)
            
            clip_features = video_features @ text_features.T
            val_clip_features = val_video_features @ text_features.T

            del video_features, text_features, val_video_features
    ...
    if args.hard_label is None:
        ...
        del clip_features
        with torch.no_grad():
            video_features = torch.load(clip_save_name, map_location="cpu").float()
            video_features /= torch.norm(video_features, dim=1, keepdim=True)

            text_features = torch.load(text_save_name, map_location="cpu").float()[(highest>args.clip_cutoff) & (concepts != '')]
            text_features /= torch.norm(text_features, dim=1, keepdim=True)
        
            clip_features = video_features @ text_features.T
            del video_features, text_features
    ...
    #     val_c -= train_mean
    #     val_c /= train_std #
    # def train_classification_layer 부분
    ...
    # train_cocept_layer => 이게 learning_concept_layer.py에 구현되어있음
    if args.hard_label is None: # using dual encoder
        train_cocept_layer(args,
                        concepts, 
                        target_features,
                        val_target_features,
                        clip_features,
