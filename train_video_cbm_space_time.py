...
def train_cbm_and_save(args):
    ...
    s_text = '/data/jong980812/project/Video-CBM/results/origin_ssv2/finetune_internvid_ost_spatio/activation/ssv2_ost_spatio_concepts_ViT-B16.pt'
    t_text= '/data/jong980812/project/Video-CBM/results/origin_ssv2/finetune_internvid_ost_temporal/activation/ssv2_ost_temporal_concepts_ViT-B16.pt'
    if dist.get_rank()==0:
        proj_layers={}
        for j,text_save_name in enumerate([s_text,t_text]):
        #load features
            concept_set_path = '/data/jong980812/project/Video-CBM/data/concept_sets/ssv2_ost_spatio_concepts.txt' if j==0 else '/data/jong980812/project/Video-CBM/data/concept_sets/ssv2_ost_temporal_concepts.txt'
            with open(concept_set_path) as f:
                concepts = f.read().split("\n")
            print(f'****{j} th proj layer****')
            with torch.no_grad():
            ...
            print(f"Deleting {len(concepts)} Concepts from {original_n_concept}")
            if j==0:
                proj_layers['s']=(proj_layer,interpretable,len(concepts))
            else:
                proj_layers['t']=(proj_layer,interpretable,len(concepts))
            del clip_features, val_clip_features
        
        W_c_s = proj_layers['s'][0].weight[proj_layers['s'][1]]
        W_c_t = proj_layers['t'][0].weight[proj_layers['t'][1]]
        proj_layer_s = torch.nn.Linear(in_features=target_features.shape[1], out_features=proj_layers['s'][2], bias=False)
        proj_layer_t = torch.nn.Linear(in_features=target_features.shape[1], out_features=proj_layers['t'][2], bias=False)
        proj_layer_s.load_state_dict({"weight":W_c_s})
        proj_layer_t.load_state_dict({"weight":W_c_t})
        
        train_targets = data_utils.get_targets_only(d_train,args)   
        val_targets = data_utils.get_targets_only(d_val,args)
        
        with torch.no_grad():
            train_c_s = proj_layer_s(target_features.detach())#B,C
            val_c_s = proj_layer_s(val_target_features.detach())
            
            train_c_t = proj_layer_t(target_features.detach())
            val_c_t = proj_layer_t(val_target_features.detach())
            
            train_c = torch.cat([train_c_s,train_c_t],dim=-1)
            val_c = torch.cat([val_c_s,val_c_t],dim=-1)
